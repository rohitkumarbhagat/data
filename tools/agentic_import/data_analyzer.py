...
flags.DEFINE_boolean('skip_confirmation', False,
                     'Skip user confirmation before starting data analysis')
...
flags.DEFINE_string(
    ...
    'Output path prefix for generated files (default: output/analysis)')
...
class DataAnalyzerConfig:
    ...
    output_path: str = 'output/analysis'


class DataAnalyzer:
    """Analyzer for dataset understanding using Gemini LLM."""

    def __init__(self, config: DataAnalyzerConfig):
        ...
        # Generate analyzer_run_id with timestamp for this run
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self._analyzer_run_id = f"analyzer_{timestamp}"

        # Create run directory structure
        self._run_dir = os.path.join(self._datacommons_dir, 'runs',
                                     self._analyzer_run_id)
    ...
    def _get_user_confirmation(self, prompt_file: str) -> bool:
        """Ask user for confirmation before starting data analysis.
        ...
        while True:
            try:
                response = input(
                    "Ready to start data analysis? (y/n): ").strip().lower()
                if response in ['y', 'yes']:
                    return True
                elif response in ['n', 'no']:
                    print("Data analysis cancelled by user.")
                    return False
                else:
                    print("Please enter 'y' or 'n'.")
            except KeyboardInterrupt:
                print("\nData analysis cancelled by user.")
                return False

    def generate(self):
        """Generate dataset analysis using Gemini LLM."""
        ...
        if not self._config.data_config.input_data:
            raise ValueError(
                "Configuration must have at least one input data entry")

        # Validate single file input for now
        if len(self._config.data_config.input_data) != 1:
            raise ValueError(
                f"Currently only single data file is supported. "
        ...
        if self._config.dry_run:
            logging.info(
                ...
                "Skipping analysis execution.", prompt_file)
            return
        ...
        if not self._config.skip_confirmation:
            if not self._get_user_confirmation(prompt_file):
                logging.info("Data analysis cancelled by user.")
                return
    ...
    def _generate_prompt(self) -> str:
        """Generate prompt from Jinja2 template using data configuration.
...
def prepare_config() -> DataAnalyzerConfig:
    ...
    return DataAnalyzerConfig(data_config=data_config,
